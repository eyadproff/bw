...
public class CoreFxController
{
	...
	private void showPage(String formKey, String taskId, Map<String, Object> inputData)
	{
		...
		else
		{
			...
			try
			{
				bodyFxController = (BodyFxController) Class.forName(formKey).newInstance();
			}
	...
	private void showErrorDialogAndWait(Image appIcon, String errorCode, Exception exception, String... additionalErrorText)
	{
		...
		if(additionalErrorText != null) LOGGER.error("{}: " + errorsBundle.getString(errorCode + ".internal"), errorCode, additionalErrorText);
		else LOGGER.error("{}: {}", errorCode, errorsBundle.getString(errorCode + ".internal"));
	...
	private boolean showOldPage(BodyFxController bodyFxController, String taskId, Map<String, Object> inputData, GuiLanguage language, StateBundle stateBundle)
	{
		...
		{
			if(newBodyController == null) LOGGER.error("newBodyController = null");
			else LOGGER.error("newBodyController type = {}", newBodyController.getClass().getName());
	...
	{
		Object oldBodyController = guiState.getBodyController();
		
		if(oldBodyController instanceof LanguageSwitchingController &&
				oldBodyController instanceof BodyFxController) // should always be true
		{
			BodyFxController bodyFxController = (BodyFxController) oldBodyController;
			String taskId = bodyFxController.getTaskId();
			Map<String, Object> inputData = bodyFxController.getInputData();
			boolean success = showOldPage(bodyFxController, taskId, inputData, guiState.getLanguage(), stateBundle);
			if(!success) return false;
		}
		else // should never happen
		{
			if(oldBodyController == null) LOGGER.error("oldBodyController = null");
			else LOGGER.error("oldBodyController type = {}", oldBodyController.getClass().getName());
