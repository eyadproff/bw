...
public class CoreFxController extends FxControllerBase implements IdleMonitorRegisterer, PersistableEntity
{
	...
	private Map<Class<?>, ResourceBundle> resourceBundlesMap;
	...
	public ResourceBundle getResourceBundleByClass(Class<?> clazz)
	{
		return resourceBundlesMap.get(clazz);
	}
	...
	protected void initialize()
	{
		...
		Map<String, ResourceBundle> cache = new HashMap<>();
		
		Map<Class<?>, WithResourceBundle> classesWithResourceBundles = Context.getClassesWithResourceBundles();
		for(Entry<Class<?>, WithResourceBundle> entry : classesWithResourceBundles.entrySet())
		{
			Class<?> clazz = entry.getKey();
			WithResourceBundle withResourceBundle = entry.getValue();
			
			// check for bundles/strings on the same level or one level down
			
			String path = withResourceBundle.path();
			...
			{
				defaultPath = true;
				parentPackageName = clazz.getPackage().getName();
				path = parentPackageName.replace(".", "/") + "/bundles/strings";
			}
			
			ResourceBundle resourceBundle = cache.get(path);
			
			if(resourceBundle == null)
			{
				try
				{
					resourceBundle = ResourceBundle.getBundle(path, Context.getGuiLanguage().getLocale(),
					                                          new UTF8Control());
					cache.put(path, resourceBundle);
				}
				catch(MissingResourceException e)
				{
					if(defaultPath)
					{
						...
						if(resourceBundle == null)
						{
							...
							{
								String errorCode = CoreErrorCodes.C002_00023.getCode();
								String[] errorDetails = {"Failed to load the resource bundle! path = = " + path};
								showErrorDialog(errorCode, e1, errorDetails);
								return;
							}
						}
					}
			...
			resourceBundlesMap.put(clazz, resourceBundle);
		}
